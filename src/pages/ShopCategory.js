...
const ShopCategory = ({ category, banner }) => {
  ...
  // Real product data for each category with actual image URLs
  const categoryProducts = {
    men: getMenProducts(),
    women: getWomenProducts(),
    kids: getKidsProducts(),
    electronics: getElectronicsProducts(),
    appliances: getAppliancesProducts()
  };

  // Get products for current category
  const filteredProducts = categoryProducts[category.toLowerCase()] || 
                         Products.filter(item => category.toLowerCase() === item.category.toLowerCase());
